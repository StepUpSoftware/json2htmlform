...
var attributes = {
    "input" : ['type', 'name', 'id', 'placeholder', 'value', 'label', 'text'],
    "textarea" : ['type', 'name', 'id', 'placeholder', 'label'],
    "select" : ["options", "optgroups", "label", "id"],
    "button" : ['type', 'name', 'id', 'value', 'text']
};
...
var createTags = function(item) {
    ...
    if (formtags.indexOf(tag) !== -1) {
        ...
        if (['input', 'textarea', 'label', 'button'].indexOf(tag) !== -1) {
            ...
            entry += (attribute + '>' + (item.text ? item.text : '') + '</' + tag + '>');
